...
> ⚠️ **本文已不是当前交接**：请先读 **`HANDOFF_V3.md`**（2026-09-17，含最新架构/主题系统/路线/陷阱）。
> 本文保留作为**历史故障取证库**——58 次 CI 失败的逐条分析、cdb/pluginval/WER 取证工作流、成品安装（UAC）脚本；要查"某个红 Run 怎么挂的"或装成品时再翻这里。
...
| **P2 效果器参数（第一批 8 类型）** | Run #108-#111 四连修（include 路径层级 ×2、`juce::Slider::rotarySliderOutlineColourId`、**std::vector<juce::Component> 不可行（Component 非拷贝/非移动）→ juce::OwnedArray**、EffectParamPanel ctor 传 `EffectSlot*` vs `EffectBase*`、布尔参数 round-trip 探测值语义）→ **Run #112**（`34136703292`，HEAD `cf6c407`）全绿：**551 用例（+9 round-trip +1 panel paint）0 失败 + strictness 2 绿**。实现：EffectBase 通用参数面 + EffectParamRegistry（14 类型统一适配器，8 类型参数表，**ProcessorStore 14 短名+14 别名工厂全部改道 Registry**）+ EffectParamPanel 展开 UI（死控件 modSlider_ → ▾ 展开）。成品 `artifacts/112/`
| **P2 第二批 + IMPORT + strictness 3** | **Run #114**（`34569415337`，HEAD `5023849`，2026-09-11）全绿：**557 用例（14 类型全参数表 round-trip ×15 + 541 基线 + 1 panel paint）0 失败 + pluginval strictness 3一次通过**。EQ 9 参数（getBand/setXBand 组合更新）、AutoTune/Flanger/Phaser/RingMod/StereoWidener 补表、Distortion TYPE/Saturation MODE/RingMod WAVEFORM 枚举滑条（EffectParamSpec.values → tooltip）、标题栏 IMPORT 大按钮（=LOAD 同路径，"导入难找"对策）、EffectParamSpec 增 values 字段。成品 `artifacts/114/`；**VST3 重装被 UAC 取消 ×2（用户侧）——装与清缓存的命令模式见 §8/本行**，REAPER 缓存已清 |
| **P3 分页信息架构** | **Run #116**（`34575806492`，HEAD `b2ea94f`）全绿：**557 用例不变 + strictness 3 绿（editor 冷 312ms/热 238ms）**。主区 = pinned 频谱（顶部 ~34%，视图下拉移到频谱右上内嵌，LIVE 默认不变、切页不隐藏）+ PageTabs（自绘 6 钮，src/gui/panels/PageTabs.h）+ 内容页：TIMBRE(A/B+BLEND+XY) / FILTER / MOD(宏行+滚动调制指派) / SEQ(终于全宽) / FX / MASTER(UNISON|VOICE|ARP|TRANSPORT|MASTER，用户裁定并入)。setActivePage 只切可见性；timer/MidiLearn 与页面无关；Regions 缩为 5 块；computeRegions 硬化（频谱≥120、内容≥120）扛 pluginval 极限尺寸。spec=docs/superpowers/specs/2026-09-11-p3-tabbed-layout-design.md。**成品 artifact "AnaPlug-windows-latest"(run 116) 未下载/未装**（用户暂停，需要时从 run 34575806492 拉，模式同 §8+本表历次 UAC 安装） |
