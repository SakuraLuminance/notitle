...
TEST_CASE("MidiLearn - addMapping and getMappedValue range scaling", "[midi][mapping]")
{
    ...
    SECTION("CC at minimum value (0) → min of range")
    {
        ...
        ml.processMidi(msg);
        REQUIRE(volTarget.load() == Approx(0.0f).margin(0.001f));
    }
    ...
    SECTION("CC at centre value (64) → midpoint of range")
    {
        ...
        REQUIRE(volTarget.load() == Approx(64.0f / 127.0f).margin(0.001f));
    }
    ...
    SECTION("CC at maximum value (127) → max of range")
    {
        ...
        ml.processMidi(msg);
        REQUIRE(volTarget.load() == Approx(1.0f).margin(0.001f));
    }
}
...
TEST_CASE("MidiLearn - addMapping with custom min/max range", "[midi][mapping]")
{
    ...
    ml.processMidi(msg);
    REQUIRE(filterTarget.load() == Approx(200.0f).margin(0.001f));

    auto msg2 = juce::MidiMessage::controllerEvent(1, 1, 127);
    ml.processMidi(msg2);
    REQUIRE(filterTarget.load() == Approx(20000.0f).margin(0.001f));
    ...
    REQUIRE(filterTarget.load() == Approx(expected).margin(0.5f));
}
...
TEST_CASE("MidiLearn - startLearn captures next CC", "[midi][learn]")
{
    ...
    REQUIRE(reverbTarget.load() == Approx(expected).margin(0.001f));
}
...
TEST_CASE("MidiLearn - non-controller messages are ignored", "[midi][edge]")
{
    ...
    REQUIRE(volTarget.load() == Approx(0.0f));
}
